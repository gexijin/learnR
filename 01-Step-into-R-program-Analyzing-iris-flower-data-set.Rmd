...
- Data frames and introduce our first dataset.
- The basic ways of analyzing data with R: import data, check data types, attach, and then plot and analyze using column names.

 
## Getting started

1. Install R from [www.R-project.org](www.R-project.org). Choose the cloud server or any U.S. mirror site.
2. Install RStudio Desktop from [www.RStudio.com](www.RStudio.com). Rstudio uses the R software you just installed in the background, but provides a more user-friendly interface. R commands can be typed directly into the “Console” window. Or you can enter them in the “R Script” window and click the “Run” button. If you have an older version of R or Rstudio, I will recommend to upgrade to the new version.
3. Alternatively, you can use R and Rstudio through a web browser via [Rstudio Cloud.](https://rstudio.cloud/) Some universities also host their own Rstudio server on more powerful computers.  
...
The first 4 columns contain **numeric** values. The last one is species information as **character** values. This is an important distinction, as we cannot add or subtract character values. Sometimes we need to overwrite data types guessed by R, which can be done during the data importing process. For example, sometimes we use 1 to code for male and 0 for female. These are essentially categories; Values like 1.6 make no sense. On the other hand we sometimes have columns like student IDs that looks like numbers but they are essentially character values as the average student ID makes no sense.  

The imported data is stored in a **data frame** called *iris*, which contains both numeric and character columns. Individual values in a data frame can be accessed using row and column indices.
Another data structure is **matrix** which contains only one type of variables. More discussion about data frame, matrix, and other data structures are introduced in the later chapters.
...
c. Extract a subset which contains values in the 2nd through the 5th rows and the 1st through the 4th columns. If your data set contains less rows or columns, please choose another data set.
```
...
This is the recommended way to referring to columns. But if you are as lazy as me and do not want to type the data frame name repeatedly. This is a shortcut by attaching a dataframe the columns can be accessed directly.
```{r echo=TRUE, results='hide'}
attach(iris)
...
One disadvantage of attaching a dataset is that it will be a mess up if two datasets have the same variable names. Therefore an alternative way, in fact a better way, is using **$** sign to take the variable. For example, if we want to see the variable Petal.Length in the data set iris, here is the code we use: 

```{r}
iris$Pepal.Length #In front of the **$** is the data set, after the **$** is the variable name. 
```
...
First, let's retrieve the 150 in the nubmerspetal length column and store it in an vector called x.
```{r}
x <- Petal.Length   
```
Some might complaine that nothing happens after you type in this command. Although nothing is printed out, you have created a new **vector** called x in the memory. You can find it in the top right panel in the **Environment** tab. *The process of programming is all about creating and modifying data objects; these data objects can be printed out or operated on.*

**Vectors** holds a series of values, such as numbers and characters. R made made it very easy to do the same calculations (add, subtract, multiply,...) on all of them at once or collectively (sum, average, etc.). 
```{r}
summary(x)
```
...
In Rstudio, you can copy a plot to clipboard using the **Export** button on top of the plot area. Or you can click **zoom**,  right click on the popup plot and select **“Copy Image”**.  Then you can paste the plot into Word. If you are using native R software, instead of Rstudio, you can right click on the plots and copy as meta-file. 
...
Lag plot is a scatter plot against the same set of number with an offset of 1. Any structure in lag plot indicate non-randomness in the order in which the data is presented. We can clearly see three big clusters, indicating that values are centered around three levels sequentially.
...
There are 3 species, therefore we select 3 for the rainbow() function. The 3 colors will correspond the 3 species. The **class(Species)** returns the data structure of Species is a "factor". The **levels(Species)** function shows all levels of Species. The function **str(Species)** shows that the Species is a factor with 3 levels which are internally coded as 1, 2, and 3. 

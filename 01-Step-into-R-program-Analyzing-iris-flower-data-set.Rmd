...
matrix, and other data structures are introduced in chapter 3.
...
First, let's retrieve the 150 nubmer in the spetal length column and store it in
an vector called x.
```{r} 
x <- iris$Petal.Length   
```
...
```
There are three different groups in the scatter plot. x of one group is much smaller than the other two groups.

...
**Challenge 1j**: Investigate the relationship between sepal length and sepal width using scatter plots, correlation coefficients, test of correlation, and linear regression. Again interpret all your results in PLAIN and proper English.
...
```
**Challenge 1k**: Use boxplot and t-test to investigate whether sepal width is different between *I. versicolor* and *I. virginica*. Interpret your results. 
...
```{r fig.keep='none'}
ggplot(iris, aes(x = Sepal.Width)) + geom_histogram(binwidth=.2, colour="black", fill="lightblue") + 
    facet_grid(Species ~ .)
ggplot(iris, aes(x = Petal.Width, fill = Species)) + geom_density(alpha=.3)
```
...
```
**Challenge 1l**: Use boxplot, multiple panel histograms and density plots to investigate whether petal width is the same among three subspecies. 

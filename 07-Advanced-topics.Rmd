...
```{r include=F, out.width = "85%", fig.cap = "RMarkdown Cheet Sheet"}
knitr::include_graphics("rmarkdownCheatSheet.pdf")
```
...
```{r label, echo=F, out.width = "80%", fig.cap = "Create a new RMarkdown file"}
...
The second part of a .Rmd file is surrounded by three back ticks "`". They are called chunks of R. An eample of a chunk is shown below. The "cars" after r in the \{ \} is the name of the chunk.  
```{r echo=FALSE, out.width='100%'}
knitr::include_graphics("images//Ch7_Adv_top_3.PNG")
```

A chunk is a collection of r code. You can run each chunk by clicking the right arrow on the top-right of the chunk or by pressing the shortcut Ctrl+Shift+Enter. Then R executes the code and displays the outputs inline with the code. See the example below. 

```{r nm_cars}
summary(cars)
```

You can find manys options for chunks in the R Markdown Cheet Sheet. let's take a look at some of the chunk options that you'll use frequently.
...
```{exercise}
Do all the following Homework by using R Markdown. You should submit your .Rmd file and the final document with pdf format. (You do not submit any answer key for this exercise. )
```
...
b.	Switch the data. To replace the original dataset faithful to a new dataset iris, we need to change the dataset specified in line 40. See the hignlighted code in the following graphs. Save the changes and run the app again. 
...
Here is the output of histogram of the Sepal length in iris dataset.
...
Solve the error by deleting the last column from the data iris which reflects in the line 20 *selectInput(“cid”, “Column”, choices = colnames(iris))*. 
```

```{exercise}
Change the title from “Histogram of x” to the name of the selected variable like “Sepal.Length”. Fill in the blanks.
```
...
e.	We can build a more complex app by adding a normal distribution line by **expanding line 45: hist(x, breaks = bins, col = 'green', border = 'white')** into these:
...
```{exercise}
Fill in the blanks to solve the error message in this app by plotting a pie chart when the Species column is selected.
```
Steps:

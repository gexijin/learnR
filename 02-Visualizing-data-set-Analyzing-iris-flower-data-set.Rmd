...
```{r, fig.align='center', out.width='80%'}
plot(PL, PW, pch = 2) # pch = 2 means the marker is triangle
```


The **pch** parameter can take values from 0 to 25, each corresponding to a different type of symbol. The commonly used values and points symbols are shown in Figure \@ref(fig:pchsymbols).
...
(iris$Species)
```
This output shows that the 150 observations are classed into three species *setosa*, *versicolor*, and *virginica*. We can assign different numbers to different species by using the *as.numeric()* function.  
```{r, fig.align='center', out.width='80%'}
speciesID <- as.numeric(iris$Species)
speciesID
```
The new defined variable *speciesID* shows that numbers 1, 2, and 3 are assigned to represent the three species. Now we can assign different symbols to different species by setting *pch = speciesID*.
```{r, fig.align='center', out.width='80%'}
...
50 (*virginica*) are in crosses.

Follow the same thinking, we also change the color according to species. We
generate three colors with rainbow() function and then alternate using the species.

...
```
Packages only need to be installed once. But every time you need to use **functions or data** in a package, you have to **load** it from your hard drive. 
...
pairs(x, col = rainbow(3)[iris$Species])  # Figure 2.4
```
...
>
Write R code to generate a figure similar to Figure \@ref(fig:12-8). Show the means of petal width for each of the species with error bars corresponding to standard deviations. 
```
...
Notice that las = 2 option puts the data labels vertically. The par function with mar = c(8, 2, 2, 2)
sets the bottom, left, top and right margin sizes of the plot region as 8, 2, 2 and 2 respectively. The units of these margin numbers are lines of text. Here we set the bottom margins to 8

...
```{r fig.keep='none'}
stars(x, key.loc = c(20,0.5), draw.segments = TRUE )    
```
...
Notice that ggplot2 command span multiple lines. The first line defines the plotting space. The ending "+" signifies that another layer ( data points) of plotting is added. Also, the ggplot2 package handles a lot of the details for us. We can easily generate many different types of plots.
...
```{r fig.cap='(ref:22-1)', out.width='60%',fig.align='center'}
ggplot(data = iris) +
  aes(x = Species, y = Sepal.Length, color = Species) +
  geom_boxplot() +
...
![Animated GIF](https://github.com/gexijin/learnR/raw/master/images/drawing.gif)
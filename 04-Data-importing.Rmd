...
![Animated GIF for screen shot.](https://github.com/gexijin/learnR/raw/master/images/dataImport.gif)
...
This is done outside of Rstudio on Windows or Mac interface. Download the heartatk4R.txt file from this page http://statland.org/R/RC/tables4R.htm. It is a tab-deliminated text file, meaning the different columns are separated by tab. We also need to get some context about the data and how it is collected. At the minimim level, we need to know the meaning of each columns.
...
As shown in Figure \@ref(fig:9-2), you can click on the automatically guessed datas type under each of the column names as shown in Figure \@ref(fig:9-2). By selecting “character” from the drop down, you can successfully format this column as a character.


On the other hand, LOS (length of stay in days) and AGE should be numbers. But because 10 is presented as "0010", these columns are automatically recognized as characters. We have to force R to read these columns as intergers by clicking on the column title and select **integer**. 
...
**arrange, mutate, filter** are called **action verbs**. For more action verbs, see [dplyr cheat Sheet](https://www.rstudio.com/resources/cheatsheets/#dplyr). 

...
```{r fig.keep='none'} 
ggplot(df, aes(x = AGE, color = SEX)) + geom_density(alpha = 0.3)  # Figure 15.4A
```
...
```{r fig.keep='none'}
ggplot(df, aes(x = AGE, fill = SEX)) + geom_density(alpha = 0.3)  # Figure 15.4B
```
...
ggplot(df, aes(x = AGE, fill = SEX)) + 
       geom_density(alpha = 0.3) + facet_grid(DRG ~.)  # Figure 15.4C
```
...
**Challenge 2**:  Create density plots like Figure \@ref(fig:15-5) to compare the distribution of length of hospital stay (LOS) for patients with different DRG groups, separately for men and women. Offer interpretation in the context of the dataset. Limit the x-axis to 0 to 20. 
...
ggplot(df, aes(x = LOS,fill = DRG)) + geom_density(alpha = 0.3) + 
       facet_grid(SEX ~ .) + xlim(0, 20)  # Figure 15.5
```
...
ggplot(df, aes(x = SEX, y = AGE)) + geom_boxplot() + facet_grid(DRG ~ .)  # Figure 15.6A
ggplot(df, aes(x = SEX, y = AGE)) + geom_violin() + facet_grid(DRG ~ .)  # Figure 15.6B
```
...
**Challenge 3**: Generate a violin plot like Figure \@ref(fig:15-6)C to compare the distribution of length of hospital stay among patients with different prognosis outcomes (DRG), separately for men and women. Interpret your result. Note the axes labels are customized.
...
ggplot(df, aes(x = DRG, y = LOS)) + geom_violin()  + facet_grid(SEX ~ .) + 
       labs(x = "DRG groups", y = "Length of Stay(days)") + ylim(0, 20)  # Figure 15.6C
```
...
**Challenge 4**. Generate Figure \@ref(fig:15-9) and offer your interpretation. Hint: Modify both the summarizing script and the plotting script. 
...
Be using pair-wised correlation analysis, we found that women have a much higher mortality rate than men due to heart attack. We also found that women are much older than men. These are obviously confounding effects.
...
**Challenge 10**. Use multiple linear regression to investigate the factors associated with length of stay. Obvious we need to exclude charges in our model.   Interpret your results.

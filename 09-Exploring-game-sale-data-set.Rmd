...
# Keep games with ratings
game <- subset(tem, Rating != "")
...
game[sales_columns] <- lapply(game[sales_columns], function(x) x * 1000000 + 1)
...
sales_columns <- c("User_Score", "Critic_Count", "User_Count")
game[sales_columns] <- lapply(game[sales_columns], function(x) as.numeric(as.character(x)))

# Set category variables as factor variables 
sales_columns <- c("Year_of_Release", "Name", "Platform", "Genre", "Publisher", "Developer", "Rating")
game[sales_columns] <- lapply(game[sales_columns], function(x) as.factor(as.character(x)))

# Format column names for clarity
colnames(game) <- c("Name", "Platform", "Year.Release", "Genre", "Publisher", 
                     "NA.Sales", "EU.Sales", "JP.Sales", "Other.Sales", 
                     "Global.Sales", "Critic.Score", "Critic.Count", 
...
Our pre-analysis shows that these variables are shy away from normality, notably the sales and score counts. To remedy this, we shall wield the power of logarithms! let's bind the log-transformed variables with their original counterparts.
```{r}
sales_columns <- c("NA.Sales", "EU.Sales", "JP.Sales", "Other.Sales", 
                   "Global.Sales", "Critic.Count", "User.Count")
game[paste0(sales_columns, ".Log")] <- lapply(game[sales_columns], function(x) log(x))
```
...
game <- game %>%
  mutate(Platform.type = case_when(
                         ...
                         Platform %in% playstation ~ "Playstation",
                         TRUE ~ "Others"))

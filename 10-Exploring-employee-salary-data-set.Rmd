...
Here is the summary for these variables from the sampled data set. We will pick full time employee as our goal and adjust the range of the interested variables to make them reasonable.
...
plot(PersonOccurCount, col = rainbow(30), 
     xlab = "Total Occurance of the Employee", ylab = "Count of the employee", 
     main = "The counts of employees who are on the pay list in 2017")
```

The most more employees occurrence is 26. We will focus on the employees who have 26 paychecks for whole year in 2017.
...
barplot(counts, col = rainbow(26), axis.lty = 1, xlab = "Check Date", ylab = "Count", 
        main = "The counts of checks for check date in 2017")
```
...
dupliSex <- t2[duplicated(t2$EmplId.Empl.Rcd), ]
dupliSex
```
...
t3 <- raw26 %>% 
  ...
  summarise(Total.Gross = sum(Tot.Gross ))
dupliEthnic.Grp <- t3[duplicated(t3$EmplId.Empl.Rcd), ]
dupliEthnic.Grp
```
...
t3 <- salary %>% 
  ...
  summarise(Total.Gross = sum(Tot.Gross ))
dupliUnion <- t3[duplicated(t3$EmplId.Empl.Rcd), ]
...
topUnion <- salary %>%
  ...
  head(10)
library(knitr)
kable(data.frame(top10Union, topUnion), booktabs=TRUE)
```
...
ggplot(aes(x = Age, y = Avg.Gross)) +
  ...
  labs(y = "Average gross income")
      #title = "Average gross income distribution by age") 
...
ggplot() + 
  ...
  labs(title = "Distribution of Gross.Log by age group") +
  theme(plot.title = element_text(hjust = 0.5))
```
...
ggplot(aes(x = Age, y = Avg.Gross, fill = Union.Descr)) +
  ...
  scale_x_continuous(breaks = seq(10, 100, 10)) +
  labs(y = "Average Gross Percentage", 
       title = "Average gross percentage of Union Descriptions by age group") +
...
Both plots tell us that for each ethnic group, male have higher average gross than female. Amind male have more bigger log of gross than Amind female. From the point and line plot we can see the same situation for Pacif ethnic. Because of the tiny amount of PACIF employees, there is no ridge showing in the plot.
...
ggplot(df, aes(x = Category, y = Sd.Gross)) + 
  ...
  facet_wrap(~ Sex) +
  labs(title = "Diverging Bars of stanardized log of gross income", 
       x = "Union Descreption", y = "Standardized log of gross") + 
  coord_flip() +
...
df <- merge(df1, df2, by="Gross.Type") %>% arrange(desc(Avg.Log.Gross)) %>%
      mutate(Std.Gross = Avg.Log.Gross/Avg.Log.Gross[1])
df <- round(df[, 2:6], 2)  # round all numeric variables to decimal 2
...
High gross group occupy 6% of the total employees. If we standardize the high group's gross as one, around 68% employees earned average gross 0.93, and around 26% employees earned 0.87. The gross decrease straightly. The real average total gross change from 6307 dollars/Bi-weekly to 3380 to 2066 for high, middle and low gross group.
...
1  Total Gross is not statistically normally distributed. It skewed to left. After taking log transformation, it is more close to normality.

2  The elder the age, the higher the gross income, except for age group between 80 and 90.

3  On average, males earn more than females in the same age, ethnic, union.

4  The means of gross are significantly different for different ethnic groups. On average, Asian and white are the top two groups. PACIF and unknown are the last.

5  The real average total gross change from 6307 dollars bi-weekly to 3380, and to 2066 for high , middle and low gross group. High, middle and low gross group occupy 6%, 68%, and 26% of the total employees. White male occupy much bigger portion in high gross group, especially the elder white male.

6  Age group, Union description, Sex, and Ethnic group together will explained 50% of log variance of gross. 
